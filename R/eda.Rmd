...
temp <- books %>% 
  ...
  filter(str_detect(ngram, pattern = all_names$name)) %>% 
  select(ngram) %>% 
  apply(., 1, function(x) str_extract(x, pattern = all_names$name))
  #mutate(chars = str_extract_all(ngram, pattern = all_names$name)) #%>% 

temp1 <- temp %>% 
  t() %>% 
  as.tibble() %>% 
  unite("names", V1:V3180, sep = "")
