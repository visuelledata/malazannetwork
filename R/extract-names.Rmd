...
char_appearances <- list.files("data/network_data") %>% 
  map_df(~read_rds(paste0("data/network_data/", .))) %>% 
  remove_NAs() %>% 
  filter(names != "") %>%
  standardize_names_net() %>% 
...
write_rds(char_appearances, "data/char_appearances.rds", compress = "gz")
write_csv(char_appearances, "data/char_appearances.csv")
